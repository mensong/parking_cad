...
public:
	...
	//获取指定图层上所有实体ID
	static AcDbObjectIdArray GetAllEntityId(const TCHAR* layername, AcDbDatabase *pDb = acdbCurDwg());
	...
	//获取当前图层所有文字样式
	static void GetALLTextStyle(std::vector<CString>& textStyle, AcDbDatabase *pDb = acdbCurDwg());
	////字符串分割到数组
